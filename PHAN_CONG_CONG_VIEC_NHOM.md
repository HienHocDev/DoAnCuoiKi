...
Bạn 2: Dự án, chi tiết dự án, Firestore cho projects
Bạn 3: Công việc, lịch, thông báo, báo cáo/charts, Firestore cho tasks
```

## Bạn 1: LoginActivity, Firebase Auth, User/Profile
...
Trong `LoginActivity.java`, hiện tại nút `Đăng nhập` đang chuyển thẳng vào trang chủ.
...
5. Nếu thành công thì chuyển vào HomeActivity.
...
Trong `AccountActivity.java`, nút `Đăng xuất` hiện đang quay về màn hình login.
...
Trong `AccountActivity.java` và `activity_account.xml`, hiện tại đang hiển thị cứng:
...
## Bạn 2: ProjectsActivity, AddProjectActivity, ProjectDetailActivity, Firestore cho Projects
...
Lưu ý: bạn 2 có thể cần đọc số lượng task để tính tiến độ dự án, nhưng `Task.java` và `TaskRepository.java` do bạn 3 phụ trách. Khi cần, bạn 2 gọi hàm do bạn 3 viết thay vì tự tạo thêm repository task riêng.
...
Trong `AddProjectActivity.java` và `activity_add_project.xml`, phần thêm dự án hiện chỉ là giao diện.
...
8. Sau khi lưu thành công, quay về ProjectsActivity.
```
...
Trong `ProjectsActivity.java`, hiện đang dùng `SampleData`.
...
Ô tìm kiếm trong `activity_projects.xml` hiện là khung giao diện, xử lý lọc sẽ làm trong `ProjectsActivity.java`.
...
Trong `ProjectDetailActivity.java` và `activity_project_detail.xml`, phần chi tiết dự án đang hiển thị cứng Website bán hàng.
...
4. Load thông tin tổng quan dự án.
5. Phối hợp với bạn 3 để hiển thị danh sách task thuộc projectId.
```
...
### 9. Cập nhật HomeActivity

Trong `HomeActivity.java`, hiện tổng số dự án, công việc, hoàn thành đang là số cứng.
...
5. HomeActivity có số liệu thật.
6. Có Project.java và ProjectRepository.java.
7. Không tự tạo TaskRepository riêng để tránh trùng phần của bạn 3.
```

## Bạn 3: TasksActivity, CalendarActivity, Notifications, Reports/Charts
...
Trong `TasksActivity.java`, hiện đang dùng dữ liệu cứng.
...
Nút `+` trong `TasksActivity.java` đang để "đang phát triển".
...
Trong `CalendarActivity.java`, hiện lịch và task trong ngày đang là dữ liệu cứng.
...
Trong `NotificationsActivity.java`, phần thông báo đang là dữ liệu cứng.
...
Trong `ReportActivity.java`, phần báo cáo hiện là dữ liệu cứng.
...
Trong `ReportActivity.java`, phần hiệu suất thành viên hiện là dữ liệu cứng.
